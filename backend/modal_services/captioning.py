...
def serve():
    ...
    app = fastapi.FastAPI(
        title="Qwen3-VL Captioning API",
        description="OpenAI-compatible API for satellite image captioning using Qwen3-VL",
        version="1.0.0",
        docs_url="/docs",
        redoc_url="/redoc",
    )
    ...
    @app.post(
        ...
        description="Generate a caption for a satellite image using OpenAI-compatible format."
    )
    ...
    @app.post(
        ...
        summary="Async Chat Completion",
        description="Submit an async inference job and get a call_id for polling."
    )
    async def chat_async(req: ChatRequest):
        ...
        try:
            ...
            try:
                ...
                return JSONResponse(
                    content={
                        "error": f"Function not found. Deploy with 'modal deploy captioning.py'. Error: {str(e)}"
                    },
                    status_code=503
                )
