...
export function ChatInput({ onSend, isProcessing, mode, setMode, onFocusChange }: ChatInputProps) {
  ...
  const uploadImage = async () => {
    ...
    const res = await fetch(`${BACKEND_URL}${ROUTES.IMAGE_UPLOAD}`, {
      method: "POST",
      body: formData,
    });
  ...
  const handleSendClick = async () => {
    if ((content.trim() || selectedImage) && !isProcessing) {
      const image_url = selectedImage ? await uploadImage() : undefined;
      const data = {
        sender: "user",
        text: content,
        imageurl: image_url,
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col">
          ...
          <div className="relative">
            ...
            <textarea
              ...
              rows={1}
              className="w-full bg-transparent text-cyan-100 placeholder-cyan-700/50 px-4 py-4 focus:outline-none resize-none scrollbar-none min-h-[56px] text-[15px] relative z-10"
          ...
          <div className="flex justify-between items-center px-3 pb-3 pt-1">
            ...
            {/* Action Buttons - Record, Image, Send */}
      ...
      <div className="mt-4 flex justify-center md:hidden">
        <div className="inline-flex items-center gap-1.5 bg-gradient-to-r from-cyan-500/20 to-blue-600/20 backdrop-blur-md border border-cyan-500/40 rounded-full p-1 shadow-lg shadow-cyan-500/30">
          <ModePill
            active={mode === "question"}
            icon={<MessageSquare size={13} />}
            label="VQA"
